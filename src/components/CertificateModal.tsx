...
export const CertificateModal: React.FC<CertificateModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 animate-in fade-in duration-150">
      <div className="bg-white rounded-lg max-w-xl w-full overflow-hidden border border-black/10 shadow-xl flex flex-col max-h-[90vh]">
        
        {/* Header bar */}
        <div className="bg-white text-black p-4 sm:p-5 flex items-center justify-between border-b border-black/10 shrink-0">
          <div className="flex items-center space-x-2.5">
            <Award className="w-5 h-5 text-black/50" />
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 bg-black/[0.02]">
          <div
            ref={certRef}
            className="bg-white border-4 border-double border-black p-6 sm:p-8 rounded-xl shadow-md relative text-center text-black"
          >
            ...
            <div className="space-y-1">
              <div className="w-12 h-12 mx-auto rounded-lg bg-black text-black/60 border border-black flex items-center justify-center font-bold text-sm font-mono shadow-xs">
                OB
              </div>
              <div className="text-[10px] uppercase tracking-widest text-black/55 font-medium">
                KOMUNITAS BUILDER INDONESIA
              </div>
              <div className="text-sm font-bold tracking-wide text-black">
                OMPRENGBID LEADERBOARD & SHOWCASE
              </div>
              <div className="text-[10px] text-black/60 font-semibold">
            ...
            <div className="my-5 border-t border-b border-black/10 py-4 space-y-2">
              <div className="text-xs uppercase tracking-wider text-black/55 font-semibold">
              ...
              <div className="bg-black/[0.02] border border-black/10 p-3 rounded-xl inline-block mx-auto mt-2">
                <div className="text-2xl sm:text-3xl font-bold font-mono text-black">
                  {score} <span className="text-sm font-normal text-black/65 font-sans">ompreng</span>
                </div>
                <div className="text-xs text-black/55 font-mono mt-0.5">
                  Ketinggian Menara: <strong className="text-black">{heightMeters} Meter</strong>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between text-left pt-2 text-xs">
              <div>
                <div className="font-mono text-black/55 text-[11px]">No. Seri Dokumen:</div>
                <div className="font-mono font-medium text-black text-xs">{certNumber}</div>
                <div className="text-black/55 text-[11px] mt-0.5">Diterbitkan: {dateStr}</div>
              </div>

              {/* Official Seal Badge */}
              <div className="text-center">
                <div className="w-14 h-14 rounded-full border border-dashed border-black bg-black/[0.04] flex flex-col items-center justify-center p-1">
                  <ShieldCheck className="w-4 h-4 text-black" />
                  <span className="text-[7px] font-bold text-black leading-tight mt-0.5 tracking-wider">
                    OMPRENGBID
                  </span>
                  <span className="text-[6px] text-black/60 font-semibold">VERIFIED 2026</span>
                </div>
              </div>
            </div>
        ...
        <div className="p-4 sm:p-5 bg-white border-t border-black/10 flex items-center justify-end space-x-2.5 shrink-0">
          <button
            onClick={handlePrint}
            className="bg-white hover:bg-black/[0.03] text-black font-medium text-xs py-2 px-4 rounded-xl border border-black/18 flex items-center space-x-1.5 transition active:scale-95"
          >
            <Printer className="w-4 h-4 text-black/45" />
            <span>Cetak / PDF</span>
          </button>

          <button
            onClick={onClose}
            className="bg-black hover:bg-neutral-800 text-white font-medium text-xs py-2 px-5 rounded-xl transition active:scale-95 shadow-xs"
          >
            Tutup
          </button>
        </div>
